...
export function Portfolio() {
  const { portfolio } = useLandingPageData()
  const largeItem = portfolio.items.find((item) => item.size === 'large') || portfolio.items[0]
  const smallItems = portfolio.items.filter((item) => item.size === 'small')
      ...
      <div className="max-w-[1280px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-space-4 auto-rows-[250px]">
          {largeItem ? (
            <div className="md:col-span-8 row-span-2 group relative overflow-hidden rounded-sm border border-gray-100 bg-white">
              ...
              <div className="absolute bottom-0 left-0 p-space-6 w-full">
                <h3 className="font-h1 text-h1 text-white uppercase mb-2">{largeItem.title}</h3>
          ...
          {smallItems.map((item) => (
            <div
              key={item.title}
              className="md:col-span-4 row-span-1 group relative overflow-hidden rounded-sm border border-gray-100 bg-white"
            >
              {item.overlayPrimary ? (
              ...
              <img
                alt={item.imageAlt}
                className={`absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 ${
                  item.overlayPrimary ? 'grayscale' : ''
                }`}
                src={item.image}
              />
              <div
                className={`absolute inset-0 bg-gradient-to-t ${
                  item.overlayPrimary ? 'from-black/90 via-black/40' : 'from-black/80 via-black/20'
                } to-transparent ${item.overlayPrimary ? 'z-20' : ''}`}
              />
              <div className={`absolute bottom-0 left-0 p-space-4 w-full ${item.overlayPrimary ? 'z-30' : ''}`}>
                <h3 className="font-h2 text-h2 text-white uppercase">{item.title}</h3>
              </div>
            </div>
          ))}
        </div>
